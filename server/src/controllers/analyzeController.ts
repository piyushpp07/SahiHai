...
const geminiModel = genAI.getGenerativeModel({ model: "models/gemini-2.0-flash" });
...
export const analyzeMedia = async (req: Request, res: Response) => {
  ...
  try {
    ...
    if (mimetype.startsWith("image/")) {
      ...
      try {
        logger.info(
          "analyzeMedia: Attempting Gemini image analysis with models/gemini-2.0-flash"
        );
      ...
      try {
        logger.info(
          "analyzeMedia: Attempting Gemini audio analysis with gemini-1.5-flash"
        );
        ...
        logger.error("❌ Gemini audio analysis failed", {
          error: geminiError.message,
        });
        return res.status(500).json({
    ...
    if (mediaType === "image" && parsedGeminiOutput.items) {
      ...
      try {
        logger.info(
          "analyzeMedia: Attempting Groq analysis with llama-3.3-70b-versatile"
        );
        ...
        try {
          ...
          logger.info(
            "✅ Groq analysis successful with fallback model mixtral-8x7b-32768"
          );
        } catch (fallbackError: any) {
          logger.error("Both Groq models failed", {
